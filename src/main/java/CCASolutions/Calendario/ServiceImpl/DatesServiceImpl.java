...
public class DatesServiceImpl implements DatesService {
    ...
    private boolean isNewMoon(LocalDateTime dateO, MoonPhasesDTO moonPhases) {
        for (MoonPhasesDTO.Response.Data m : moonPhases.getResponse().getData()) {
            if ("NewMoon".equals(m.getMoonPhase()) && LocalDateTime.parse(m.getDate()).toLocalDate().isEqual(dateO.toLocalDate())) {
                return true;
            }
        }
        return false;
    ...
    private String getVAUDay(LocalDateTime dateO, MoonPhasesDTO moonPhases) {
        if (isNewMoon(dateO, moonPhases)) return "-";

        LocalDateTime lastNewMoon = getLastNewMoon(dateO, moonPhases);
        if (lastNewMoon == null) return "?";
    ...
    private String getVAUMonth(LocalDateTime dateO, SolsticiosAndEquinocciosDTO solsticios, MoonPhasesDTO moonPhases) {
        ...
        if (lastEventName != null) {
            switch (lastEventName) {
                case "WinterSolstice": season = 1; break;
                case "VernalEquinox": season = 2; break;
                case "SummerSolstice": season = 3; break;
                case "AutumnalEquinox": season = 4; break;
            }
        }
    ...
    private Long getVAUYear(LocalDateTime dateO) {
        ...
        if (dateO.toLocalDate().isEqual(lastMeton.toLocalDate())) return 0L;
        else if (dateO.getYear() == lastMeton.getYear()) return 1L;
        else return (long) (dateO.getYear() - lastMeton.getYear());
    }
}
